...
def app():
    ...
    # +y = 50, means that the windows veritcal position will be 50 pixels below
    ...
    buttons = [
        ...
        (
            "Average Attendance",
            lambda: show("Average Attendance", avg_attendance()),
        ),
        ...
        (
            "Graph: Attendance Vs Grade",
            lambda: make_graph("attendance", "grade"),
        ),
    ]
    ...
    search_var.trace(
        "w",
        lambda *args: update_suggestions(search_var, suggestion_list),
    )
    ...
    search_entry.bind(
        "<Return>",
        lambda event: perform_search(search_var, details_label),
    )
    ...
    try:
        ...
        # mainloop ensures the window remains visible, without it will appear and
        # disappear quickly
        root.mainloop()
