...
def _fetch_df():
    # Only runs when you actually make a graph
...
# These are set later inside make_graph()
df = None
avg_grade_by_age = None
avg_att_by_age = None
avg_att_by_grade = None
# ^ putting them here stops NameError and stops anything running at import time
...
def attendance_grade():
    ...
    # Guard so polyfit doesn't crash if there's not enough data
    if len(df) >= 2:
...
def make_graph(x, y):
    ...
    # reset_index() turns the grouped index
    # back into normal columns for easy plotting
